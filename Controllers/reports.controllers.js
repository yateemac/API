...
exports.allAgreementsSMCRprt = (req, res) =>
{
    ...
    var request = new mssql.Request();
    // query to the database and get the records
    const queryStr = `SELECT J.JOBNO, J.JOBNAME, J.JDATE, S.SONO, S.SODATE, A.AGR_NO, A.PCODE CUST_CODE, S.CUST_NAME,A.CUST_NAME AS CONTACT, A.START_DATE, A.END_DATE, A.INVOICE_PERIOD, A.TOTAL AGR_AMT, CASE WHEN A.END_DATE<GETDATE() Then 'Expired' ELSE 'Running' END STATUS FROM AGREEMENTMASTER A LEFT OUTER JOIN SOMASTER S ON A.COMP_CODE=S.COMP_CODE AND A.SONO=S.SONO LEFT OUTER JOIN JOB J ON S.COMP_CODE=J.COMP_CODE AND S.JOB_NO=J.JOBNO WHERE A.COMP_CODE='01' AND J.JOBNO LIKE 'MC%' AND EXISTS (SELECT * FROM (SELECT TOP 1 A2.COMP_CODE, J2.JOBNO, A2.AGR_NO, A2.START_DATE, A2.END_DATE FROM AGREEMENTMASTER A2 LEFT OUTER JOIN SOMASTER S2 ON A2.COMP_CODE=S2.COMP_CODE AND A2.SONO=S2.SONO LEFT OUTER JOIN JOB J2 ON S2.COMP_CODE=J2.COMP_CODE AND S2.JOB_NO=J2.JOBNO WHERE A2.COMP_CODE=A.COMP_CODE AND J2.JOBNO=J.JOBNO ORDER BY A2.START_DATE DESC) D WHERE D.COMP_CODE=J.COMP_CODE AND D.JOBNO=J.JOBNO AND D.AGR_NO=A.AGR_NO);`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
...
exports.allAgreementsRprt = (req, res) =>
{
    ...
    var request = new mssql.Request();
    // query to the database and get the records
    const queryStr = `SELECT J.JOBNO, J.JOBNAME, J.JDATE, S.SONO, S.SODATE, A.AGR_NO, A.PCODE CUST_CODE, S.CUST_NAME,A.CUST_NAME AS CONTACT, A.START_DATE, A.END_DATE, A.INVOICE_PERIOD, A.TOTAL AGR_AMT FROM AGREEMENTMASTER A LEFT OUTER JOIN SOMASTER S ON A.COMP_CODE=S.COMP_CODE AND A.SONO=S.SONO LEFT OUTER JOIN JOB J ON S.COMP_CODE=J.COMP_CODE AND S.JOB_NO=J.JOBNO WHERE A.COMP_CODE='01' AND J.JOBNO LIKE 'MC%';`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
...
exports.allTradingTotalSalesRprt = (req, res) => 
{
    ...
    var request = new mssql.Request();
    // query to the database and get the records
    const queryStr = `SELECT PCODE, DESCRIPTION, SUM(-1 * TOTALQUANTITY) AS QTY, SUM(-1 * TOTALQUANTITY * UNITCOSTPRICE) AS COST, SUM(-1 * TOTALQUANTITY * UNITPRICE) AS SELLLING, SUM(-1 * LINE_QTY * LINE_UNIT_PRICE - LINE_QTY * UNITCOSTPRICE * -1) AS GROSS FROM SALES_DETAILS WHERE YEAR (TRN_DATE) = '${req.params.year}' GROUP BY DESCRIPTION, PCODE;`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
...
exports.showroomItemwiseSalesRprt = (req, res) =>
{
    ...
    const queryStr = `SELECT PCODE, DESCRIPTION, SUM(-1 * TOTALQUANTITY) AS QTY, SUM(-1 * TOTALQUANTITY * UNITCOSTPRICE) AS COST, SUM(-1 * TOTALQUANTITY * UNITPRICE) AS SELLLING, SUM(-1 * TOTALQUANTITY * UNITPRICE - TOTALQUANTITY * UNITCOSTPRICE * -1) AS GROSS FROM SALES_DETAILS WHERE DEPT IN ('RU','SU','PL') AND YEAR (TRN_DATE) = '${req.params.year}' GROUP BY DESCRIPTION, PCODE ORDER BY PCODE;`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
...
exports.monthWiseShowroomSalesRprt = (req, res) => 
{
    ...
    const queryStr = `SELECT MONTH (TRN_DATE) AS MONTH, SUM(-1 * TOTALQUANTITY) AS QTY, SUM(-1 * TOTALQUANTITY * UNITCOSTPRICE) AS COST, SUM(-1 * TOTALQUANTITY * UNITPRICE) AS SELLLING, SUM(-1 * TOTALQUANTITY * UNITPRICE - TOTALQUANTITY * UNITCOSTPRICE * -1) AS GROSS, SUM(AMOUNT) AS AMOUNT FROM SALES_DETAILS WHERE DEPT IN ('RU','SU','AP') AND YEAR (TRN_DATE) = '${req.params.year}' GROUP BY MONTH (TRN_DATE) ORDER BY MONTH (TRN_DATE);`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
...
exports.dayWiseShowroomRprt = (req, res) => 
{
    ...
    const queryStr = `SELECT TRN_DATE, PCODE, DESCRIPTION, -1 * TOTALQUANTITY AS QTY, -1 * TOTALQUANTITY * UNITCOSTPRICE AS COST, -1 * TOTALQUANTITY * UNITPRICE AS SELLLING, -1 * TOTALQUANTITY * UNITPRICE - TOTALQUANTITY * UNITCOSTPRICE * -1 AS GROSS FROM SALES_DETAILS WHERE DEPT IN ('RU','SU','AP') AND TRN_DATE = '${req.params.date}' ORDER BY TRN_DATE;`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
...
exports.dayWiseOneStopShopSalesRprt = (req, res) => 
{
    ...
    const queryStr = `SELECT TRN_DATE, PCODE, DESCRIPTION, -1 * TOTALQUANTITY AS QTY, -1 * TOTALQUANTITY * UNITCOSTPRICE AS COST, -1 * TOTALQUANTITY * UNITPRICE AS SELLLING, -1 * TOTALQUANTITY * UNITPRICE - TOTALQUANTITY * UNITCOSTPRICE * -1 AS GROSS FROM SALES_DETAILS WHERE DEPT IN ('PC') AND TRN_DATE = '${req.params.date}' ORDER BY TRN_DATE;`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
...
exports.monthWiseOneStopShopSalesRprt = (req, res) => 
{
    ...
    const queryStr = `SELECT MONTH (TRN_DATE) AS MONTH, SUM(-1 * TOTALQUANTITY) AS QTY, SUM(TOTALQUANTITY * UNITCOSTPRICE * -1) AS COST, SUM(TOTALQUANTITY * UNITPRICE * -1) AS SELLLING, SUM(-1 * TOTALQUANTITY * UNITPRICE - TOTALQUANTITY * UNITCOSTPRICE * -1) AS GROSS, SUM(AMOUNT) AS AMOUNT FROM SALES_DETAILS WHERE DEPT IN ('PC') AND YEAR (TRN_DATE) = '${req.params.year}' GROUP BY MONTH (TRN_DATE) ORDER BY MONTH (TRN_DATE);`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
...
exports.showroomTotalSalesRprt = (req, res) => 
{
    ...
    const queryStr = `SELECT SUM(TOTALQUANTITY) AS QTY, SUM(-1 * TOTALQUANTITY * UNITCOSTPRICE) AS COST, SUM(-1 * TOTALQUANTITY * UNITPRICE) AS SELLLING, SUM(-1 * TOTALQUANTITY * UNITPRICE - TOTALQUANTITY * UNITCOSTPRICE * -1) AS GROSS, SUM(AMOUNT) AS AMOUNT FROM SALES_DETAILS WHERE DEPT IN ('RU','SU','AP') AND YEAR (TRN_DATE) = '${req.params.year}';`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
...
exports.oneStopShopTotalSalesRprt = (req, res) => 
{
    ...
    const queryStr = `SELECT SUM(-1 * TOTALQUANTITY) AS QTY, SUM(TOTALQUANTITY * UNITCOSTPRICE * -1) AS COST, SUM(TOTALQUANTITY * UNITPRICE * -1) AS SELLLING, SUM(-1 * TOTALQUANTITY * UNITPRICE - TOTALQUANTITY * UNITCOSTPRICE * -1) AS GROSS, SUM(AMOUNT) AS AMOUNT FROM SALES_DETAILS WHERE DEPT IN ('PC') AND YEAR (TRN_DATE) = '${req.params.year}';`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
...
exports.serviceDeptTotalSalesRprt = (req, res) => 
{
    ...
    const queryStr = `SELECT SUM(TOTALQUANTITY * -1) AS QTY, SUM(-1 * TOTALQUANTITY * UNITCOSTPRICE) AS COST, SUM(-1 * TOTALQUANTITY * UNITPRICE) AS SELLLING, SUM(-1 * TOTALQUANTITY * UNITPRICE - TOTALQUANTITY * UNITCOSTPRICE) AS GROSS, SUM(AMOUNT) AS AMOUNT FROM SALES_DETAILS WHERE DEPT IN ('SC', 'MC', 'SJ') AND YEAR (TRN_DATE) = '${req.params.year}';`;
    request.query(queryStr, function (err, recordset) {
        if (err) console.log(err)
        else {
            if (recordset.recordset.toString() === '') {
